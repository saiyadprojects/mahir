...
DETECT_EVERY_N = 4         # Run YOLO every Nth frame and reuse the last box in
                           # between. ONNX YOLO26n at 640x640 on a Pi 5 CPU is
                           # ~300-400 ms, which is exactly why you were seeing
                           # 1.5-2.7 FPS. Stereo depth still updates EVERY frame;
                           # only the detection box is held. The box smoothing in
                           # DetectionBox hides the staleness.

# ---- COLOUR ORDER (this is the bug that made your red rose render blue) ----
# Picamera2 format names are byte-order reversed relative to how the bytes land
# in the numpy array. Requesting "RGB888" gives you an array ordered B,G,R -
# which is ALREADY what OpenCV wants. The previous code then applied another
# RGB2BGR swap on top, so red and blue were exchanged: the red rose showed as
# blue, green leaves as cyan, and - far more importantly - YOLO was being fed
# channel-swapped images, which quietly costs you detection confidence.
# Set this False only if your Picamera2 build genuinely returns RGB.
PICAM_GIVES_BGR = True

# ---- lighting robustness ----
USE_CLAHE = True           # Local contrast equalisation before stereo matching.
CLAHE_CLIP = 2.0           # Recovers texture in shadowed/underexposed regions,
CLAHE_GRID = 8             # which is where your valid-pixel % collapses.

# ---- stereo pair exposure lock ----
# Two independent Picamera2 instances run INDEPENDENT auto-exposure and
# auto-white-balance. The two eyes then differ in brightness, and SGBM - which
# matches on intensity - degrades badly. Locking both cameras to identical
# manual settings is one of the highest-value fixes on a dual-IMX219 rig.
LOCK_STEREO_EXPOSURE = True
FIXED_EXPOSURE_US = 20000  # 20 ms. Raise in dim light, lower in bright light.
FIXED_ANALOGUE_GAIN = 2.0
BRIGHTNESS_MISMATCH_WARN = 12.0   # mean-intensity delta between eyes that
                                  # triggers an on-screen warning
...
def main():
    ...
    if dual_camera_mode:
        ...
        if LOCK_STEREO_EXPOSURE:
            # Identical MANUAL exposure/gain/AWB on both eyes. Independent AE
            # between the two cameras is a classic dual-IMX219 failure: the eyes
            # end up at different brightness and SGBM, which matches on
            # intensity, quietly stops finding correspondences.
            ctrls = {"AeEnable": False, "AwbEnable": False,
                     "ExposureTime": FIXED_EXPOSURE_US,
                     "AnalogueGain": FIXED_ANALOGUE_GAIN,
                     "ColourGains": (1.8, 1.8)}
            try:
                picam_left.set_controls(ctrls)
                picam_right.set_controls(ctrls)
                time.sleep(0.5)
                print(f"[STEREO] Both eyes locked to {FIXED_EXPOSURE_US} us, "
                      f"gain {FIXED_ANALOGUE_GAIN}, AWB off.")
            except Exception as e:
                print(f"[WARN] Could not lock stereo exposure ({e}). "
                      f"Auto-exposure mismatch may degrade disparity.")
    else:
    ...
    frame_idx = 0
    last_detection = None           # held between YOLO runs (see DETECT_EVERY_N)
    ...
    try:
        while True:
            ...
            if camera_on:
                ...
                # ---- colour order (see PICAM_GIVES_BGR in CONFIG) ----
                if PICAM_GIVES_BGR:
                    left_bgr_raw, right_bgr_raw = left_raw, right_raw
                else:
                    left_bgr_raw = cv2.cvtColor(left_raw, cv2.COLOR_RGB2BGR)
                    right_bgr_raw = cv2.cvtColor(right_raw, cv2.COLOR_RGB2BGR)

                left_gray = cv2.cvtColor(left_bgr_raw, cv2.COLOR_BGR2GRAY)
                right_gray = cv2.cvtColor(right_bgr_raw, cv2.COLOR_BGR2GRAY)

                # Exposure-mismatch check: SGBM matches on intensity, so if the
                # two eyes disagree on brightness the disparity map falls apart.
                brightness_delta = abs(float(left_gray.mean()) - float(right_gray.mean()))
                ...
                rect_left_bgr = cv2.remap(left_bgr_raw, map1x, map1y, cv2.INTER_LINEAR)

                # Texture check: SGBM cannot match a flat, textureless surface no
                # matter how good the calibration is. Laplacian variance is a
                # cheap, reliable proxy for "is there anything here to match".
                texture_score = float(cv2.Laplacian(rect_left_gray, cv2.CV_64F).var())

                # ---- detection (every Nth frame - YOLO is the FPS bottleneck) ----
                frame_idx += 1
                if session is not None and (frame_idx % DETECT_EVERY_N == 0
                                            or last_detection is None):
                    dets = detect_roses(session, input_name, rect_left_bgr,
                                        CONF_THRESHOLD)
                    last_detection = dets[0] if dets else None
                best = last_detection
            ...
            if camera_on:
                # --- live diagnostics for WHY a reading failed ---
                warn_y = HALF_H - 40
                if brightness_delta > BRIGHTNESS_MISMATCH_WARN:
                    cv2.putText(display,
                                f"! EYE BRIGHTNESS MISMATCH {brightness_delta:.0f} "
                                f"- lock exposure",
                ...
                if texture_score < 80.0:
                    cv2.putText(display,
                                f"! LOW TEXTURE {texture_score:.0f} "
                                f"- nothing here for stereo to match",
                                (10, warn_y), cv2.FONT_HERSHEY_SIMPLEX,
                                0.55, (0, 165, 255), 2)

                cv2.putText(display, f"valid {valid_pct:.0f}%  disp {median_disp:.1f}"
                                     f"  tex {texture_score:.0f}  dL/R {brightness_delta:.0f}"
